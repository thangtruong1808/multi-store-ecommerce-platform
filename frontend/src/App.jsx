  ...
  return (
    <div className="mx-auto mt-8 max-w-3xl rounded-xl bg-white p-8 shadow">
      ...
      <p className="mt-3 text-base text-slate-600">
        React 18 + Vite + TailwindCSS + Redux auth state are ready.
      </p>
      <p className="mt-3 rounded bg-slate-100 p-3 text-sm">
        ...
        {!isLoading && isHydrated && isAuthenticated && `Authenticated as ${user?.email ?? 'user'}.`}
        {!isLoading && isHydrated && !isAuthenticated && 'Not authenticated (normal for first visit).'}
      </p>
      {error && <p className="mt-2 text-sm text-red-600">{error}</p>}
      <p className="mt-1 text-sm text-slate-500">
        Set <code className="rounded bg-slate-100 px-1.5 py-0.5">VITE_API_BASE_URL</code> in{' '}
        <code className="rounded bg-slate-100 px-1.5 py-0.5">frontend/.env</code> to connect to the backend.
      </p>
    </div>
  )
}
