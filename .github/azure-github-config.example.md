...
- Container images: **GitHub Container Registry** `ghcr.io/<owner>/multi-store-{api,web,postgres}:staging` (workflow pushes on `develop`).
...
For Azure Container Apps to pull without registry credentials, set each package to **Public** (`multi-store-api`, `multi-store-web`, **`multi-store-postgres`**) — GitHub → Packages → Package settings → Change visibility.
